resource "aws_security_group" "alb_sg" {
  ...
  name        = "${each.key}-dm-alb-sg"
  description = "DM ECS ALB Security Group for ${each.key}"
...
resource "aws_vpc_security_group_ingress_rule" "ecs_alb_sg_ingress_https" {
  for_each          = lookup(local.ecs_config["ecs"], "clusters", {})
...
resource "aws_vpc_security_group_egress_rule" "ecs_alb_sg_egress" {
  for_each          = lookup(local.ecs_config["ecs"], "clusters", {})
  security_group_id = aws_security_group.alb_sg[each.key].id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = -1
